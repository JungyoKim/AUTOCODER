...
async function puppet(pid) {
  ...
  const output = `1.문제설명 ${problemText} 2.입력설명. ${inputText} 3.출력설명. ${outputText} 4.입력예시. ${inputExampleText} 5.출력예시. ${outputExampleText} 출력 결과를 말하지 마십시오, 코드 내에 주석을 쓰지 마십시오, 여러게의 변수에 입력받을때 map()을 사용하지 마십시오,중요합니다! 또한 input을 따로따로 지정하십시오, 또한 코드 이외의 부가적인 말을 절대로 하지 마십시오. 가장 중요합니다!!!!!! 코드 내에 주석을 쓰지 마십시오`.replace(/\.\s+/g, ' ');
